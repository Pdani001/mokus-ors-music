	...
	async execute(wss, ws, req, data) {
        ...
        try {
            ...
            await Promise.all(Guilds.map(async guild=>{
                ...
                const voices = allChannels.filter(channel => channel != null && channel.isVoiceBased() && channel.permissionsFor(useMasterPlayer().client.user,false).has(PermissionsBitField.Flags.Connect)).map(channel => { return {name: channel.name, id: channel.id}; });
                if(voices.length > 0){
                    Channels.push({
                        "name": guild.name,
                        "id": guild.id,
